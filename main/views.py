...
from .models import CustomUser, Question,Answer,Comment,UpVote,DownVote
...
#         model = CustomUser
#         # specify the fields you want to include in the form, e.g.
#         fields = ('bio','location')
...
def home(request):
    if 'q' in request.GET:
        ...
        #quests=Question.objects.filter(title=q).order_by('-id')
    else:
    ...
    #return render(request,'home.html',{'title': 'Home Page'})
    #return render(request,'home.html' ,{'quests'})
...
def detail(request,id):
    ...
    #comments=Comment.objects.filter(answer=answers).order_by('-id')
...
def tag(request,tag):
    if 'q' in request.GET:
        q=request.GET['q']
        quests=Question.objects.filter(tag_icontains=tag).order_by('-id')
    else:
        quests=Question.objects.all().order_by('-id')
...
def profile(request):
    ...
    if request.method=='POST':
        ...
        if profileForm.is_valid():
            profileForm.save()
            messages.success(request,'Profile has been updatetd.')
...
def tags(request):
    ...
    #return HttpResponse(tag_with_count)
